...
template <class T> class Apriori
{
    private:
        set<T> set_union(const set<T> &a, const set<T> &b)
        {
            set<T> ret;
            ret.clear();
            for (typename set<T>::const_iterator itr = a.begin(); itr != a.end(); ++itr)
                ret.insert(*itr);
            for (typename set<T>::const_iterator itr = b.begin(); itr != b.end(); ++itr)
                ret.insert(*itr);
            return ret;
        }
    ...
    public:
               ...
               int maxFrequentSetCountSize = 100) 
        {
            ...
            for (int i = 2; i <= maxFrequentSetSize; ++i) {
                ...
                if (next.size() > maxFrequentSetCountSize) {
                    cout << "too much frequent items " << endl;
                    break;
                }
