...
vector<double> getGIST(IplImage * src)
{
  static double ***G = NULL;
	...
	static int numberBlocks = 4;
	static int orientationsPerScale[]= {8,8,4};
  static bool first = true;
  if (first)
  {
    cout << "GIST first called" << endl;
    first = false;
  ...
  vector<double> ret(960);
	...
	cvReleaseImage(&img);
	output.destroy();
  int W = numberBlocks * numberBlocks;
  int Nfilters = ssum(orientationsPerScale);
