					...
					struct feature*** nbrs, int max_nn_chks )
{
	...
	while( min_pq->n > 0  &&  t < max_nn_chks )
	{
		...
		for( i = 0; i < expl->n; i++ )
		{
			...
			bbfdata = (bbf_data*) malloc( sizeof( struct bbf_data ) );
			if( ! bbfdata )
			{
			...
			bbfdata->old_data = tree_feat->feature_data;
	...
	for( i = 0; i < n; i++ )
	{
		bbfdata = (bbf_data*) _nbrs[i]->feature_data;
		_nbrs[i]->feature_data = bbfdata->old_data;
		free( bbfdata );
	}
	*nbrs = _nbrs;
	return n;
...
{
	struct kd_node* kdnode;

	kdnode = (kd_node*) malloc( sizeof( struct kd_node ) );
	memset( kdnode, 0, sizeof( struct kd_node ) );
	kdnode->ki = -1;
